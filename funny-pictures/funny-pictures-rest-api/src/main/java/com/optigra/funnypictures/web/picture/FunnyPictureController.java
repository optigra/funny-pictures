...
public class FunnyPictureController extends BaseController {
     ...
     */
    @RequestMapping(value = "/{id}/funniesThumbs", method = RequestMethod.GET)
    public PagedResultResource<FunnyPictureThumbnailResource> getFunniesThumbnailsForPicture(
        ...
        return funnyPictureThumbnailFacade.getFunniesThumbnailsForPicture(id, pagedRequest);
    }

    /**
     * Method for creating funny picture. Requested method POST.
     *
     * @param funnyPictureResource Request JSON parameters with funny picture object.
    ...
    /**
     * Controller method for getting funny picture thumbnail by id and thumb type. Requested
     * method GET.
     *
     * @param id FunnyPicture Identifier
     * @return Serialized Funny Picture Resource with all required fields.
     */
    @RequestMapping(value = "/{id}/funnyThumb", method = RequestMethod.GET)
    public FunnyPictureThumbnailResource getFunnyPictureThumb(
            @PathVariable("id") final Long id,
            @RequestParam(value = "thumbnailType", defaultValue = "MEDIUM") final String type) {
        LOG.info("Getting Funny Picture Thumbnail resource with id: {} and thumbnailType: {}", id, type);
        ThumbnailType thumbnailType = ThumbnailType.valueOf(type);
        return funnyPictureThumbnailFacade.getFunnyThumbnailForFunnyPicture(id, thumbnailType);
    }
}
